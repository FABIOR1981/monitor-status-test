/**
 * js/leyenda_i18n_core.js
 * Define la funcionalidad de internacionalización (i18n) y contiene los textos en español (I18n_es)
 * para la ventana de la leyenda, ahora con códigos de estado HTTP y contenido expandible.
 */

// Objeto global 'i18n' que contiene los textos y la función de obtención
const i18n = {
    // 1. Almacén de Textos en Español
    texts: {
        // Textos generales
        'leyenda.title_browser': 'Leyenda del Monitor de Estado',
        'leyenda.main_header': 'Umbrales de Latencia y Justificación Operacional', 
        'leyenda.link_volver': 'Volver a la Aplicación', 

        // Contenido principal de la leyenda con detalles expandibles
        'leyenda.content_html': `
    ...
    /**
     * Función para obtener una cadena de texto.
     * @param {string} key La clave de la cadena (ej: 'leyenda.title_browser')
     * @returns {string} El texto correspondiente o un mensaje de error.
     */
    get: function(key) {
...
// Inicializa el texto del enlace de retorno tan pronto como el DOM esté listo
document.addEventListener('DOMContentLoaded', () => {
    const volverLink = document.getElementById('enlace-volver');
    if (volverLink && i18n.texts['leyenda.link_volver']) {
        volverLink.textContent = i18n.texts['leyenda.link_volver'];
    }
});