...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadI18n(language) {
        const file = (typeof I18N_FILES !== 'undefined' && I18N_FILES[language]) ? I18N_FILES[language] : (typeof I18N_FILES !== 'undefined' ? I18N_FILES[DEFAULT_LANG] : null);
        if (!file) return Promise.resolve();
        return new Promise((resolve, reject) => {
            const script = document.createElement('script');
            script.src = file;
            script.onload = () => resolve();
            script.onerror = () => reject(new Error(`Failed to load i18n file: ${file}`));
    ...
    try {
        ...
        console.warn('Language script failed to load, falling back to default lang.', err);
        if (idioma !== DEFAULT_LANG) {
            try { await loadI18n(DEFAULT_LANG); } catch (e) { /* ignore */ }
        }
    }
    function renderLeyendaContent(container, leyendaData) {
        ...
        (leyendaData.codigos_error || []).forEach(c => {
            ...
            const tdLabel = document.createElement('td'); tdLabel.textContent = c.label || ''; tr.appendChild(tdLabel);
            const tdDesc = document.createElement('td'); tdDesc.textContent = c.description || ''; tr.appendChild(tdDesc);
    ...
    if (typeof i18n !== 'undefined' && typeof i18n.get === 'function') {
        ...
        renderLeyendaContent(document.getElementById('contenido-leyenda'), window.TEXTOS_ACTUAL.leyenda);
